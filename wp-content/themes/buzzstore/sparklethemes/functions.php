                    ...
                    <header class="entry-header">
                        <?php if( is_product() ) {
                              ...
                              the_title( '<h2 class="entry-title">', '</h2>' ); 
                                ...
                                <h2 class="entry-title"><?php printf( esc_html__( 'Search Results for : %1$s', 'buzzstore' ), '<span>' . get_search_query() . '</span>' ); ?></h2>
                ...
                <div class="buzz-container wow zoomIn" data-wow-delay="0.3s">
                    <header class="entry-header">
                        <?php if( is_archive() || is_category() ) {
                                ...
                                <h2 class="entry-title"><?php printf( esc_html__( 'Search Results for : %s', 'buzzstore' ), '<span>' . get_search_query() . '</span>' ); ?></h2>
                                <h2 class="page-title"><?php echo esc_html__('Nothing Found','buzzstore'); ?></h2>
                            <?php }elseif( is_404() ){ ?>
                                <h2 class="entry-title"><?php echo esc_html__('404','buzzstore'); ?></h2>
                            <?php }else{
                                the_title( '<h2 class="entry-title">', '</h2>' ); 
                            }
                        ?>
...
if ( ! function_exists( 'buzzstore_quick_contact' ) ) {
	function buzzstore_quick_contact(){
          ...
          $phonenumber     = preg_replace("/[^0-9]/","",$buzzstore_quick_phone);
        ?>
    		<ul>
                <?php if( !empty( $buzzstore_map_address ) ) { ?>
                    <li>
                        <span class="icon-location-pin"></span>
                        <a target="_blank" href="https://www.google.com.np/maps/place/<?php echo esc_attr( $buzzstore_map_address ); ?>"><?php echo esc_html( $buzzstore_map_address ); ?></a>
                    </li>
                <?php } if( !empty( $buzzstore_quick_email ) ) { ?>
                    <li>
        				<span class="icon-envelope-open"></span>
        				<a href="mailto:<?php echo esc_attr( antispambot( $buzzstore_quick_email ) ); ?>"><?php echo esc_html( antispambot( $buzzstore_quick_email ) ); ?></a>
        			</li>
                <?php } if( !empty( $buzzstore_quick_phone ) ) { ?>
        			<li>
        				<span class="icon-call-out" aria-hidden="true"></span>
        				<a href="tel:<?php echo esc_attr( $phonenumber ); ?>"><?php echo esc_html( $buzzstore_quick_phone ); ?></a>
        			</li>
                <?php } ?>					
    		</ul>
        <?php
    ...
    function buzzstore_search_options(){
        ...
        if(!empty($buzz_search_options) && $buzz_search_options == 1){
            if(!empty($buzzstore_search_type) && $buzzstore_search_type == 'no' ){
               ...
               else  { echo '<div class="header-search-area">'; get_search_form(); echo '</div>';}
            } else { 
        ...
        if (is_category()) {
          ...
          for ($i = 0; $i < esc_attr( count( $breadcrumbs ) ); $i++ ) {
            ...
            if ($i != count( $breadcrumbs) - 1)
              echo ' ' . html_entity_decode( $delimiter ) . ' ';
          }
          if ($showCurrent == 1){
            echo ' ' . esc_attr($delimiter) . ' ' . wp_kses_post($before) . esc_attr(get_the_title()) . wp_kses_post($after);
          }
