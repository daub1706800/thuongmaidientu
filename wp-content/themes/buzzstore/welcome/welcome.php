    ...
    class BuzzStore_Welcome {
        ...
        public function __construct() {
            ...
            $this->tab_sections = array(
                'getting_started' => esc_html__('Getting Started', 'buzzstore'),
                'recommended_plugins' => esc_html__('Recommended Plugins', 'buzzstore'),
                'support' => esc_html__('Support', 'buzzstore'),
                'free_vs_pro' => esc_html__('Free Vs Pro', 'buzzstore')
            );
            ...
            add_action('wp_ajax_buzzstore_activate_plugin', array($this, 'activate_plugin'));
        }

        /** Trigger Welcome Message Notification */
        public function admin_notice($hook) {
            $hide_notice = get_option('buzzstore_hide_notice');
            if (!$hide_notice) {
        ...
        public function admin_notice_content() {
            ...
            if ('appearance_page_buzzstore-welcome' === $screen->id || (isset($screen->parent_file) && 'plugins.php' === $screen->parent_file && 'update' === $screen->id) || 'theme-install' === $screen->id) {
                return;
            }

            $slug = $filename = 'one-click-demo-import';
            ?>
            <div class="updated notice buzzstore-welcome-notice">
                <div class="buzzstore-welcome-notice-wrap">
                    <h2><?php esc_html_e('Congratulations!', 'buzzstore'); ?></h2>
                    <p><?php printf(esc_html__('%1$s is now installed and ready to use. You can start either by importing the ready made demo or get started by customizing it your self.', 'buzzstore'), $this->theme_name); ?></p>

                    <div class="buzzstore-welcome-info">
                        <div class="buzzstore-welcome-thumb">
                            <img src="<?php echo esc_url(get_stylesheet_directory_uri() . '/screenshot.png'); ?>" alt="<?php echo esc_attr__('BuzzStore Demo', 'buzzstore'); ?>">
                        </div>

                        <?php
                        if ('appearance_page_sparkle-theme-demo-importer' !== $screen->id) {
                            ?>
                            <div class="buzzstore-welcome-import">
                                <h3><?php esc_html_e('Import Demo', 'buzzstore'); ?></h3>
                                <p><?php esc_html_e('Click below to install and active One Click Demo Importer Plugin, It\'s help you to import demo.', 'buzzstore'); ?></p>
                        ...
                        <div class="buzzstore-welcome-getting-started">
                            <h3><?php esc_html_e('Get Started', 'buzzstore'); ?></h3>
                            <p><?php printf(esc_html__('Here you will find all the necessary links and information on how to use %s.', 'buzzstore'), $this->theme_name); ?></p>
                            <p><a href="<?php echo esc_url(admin_url('admin.php?page=buzzstore-welcome')); ?>" class="button button-primary"><?php esc_html_e('Go to Setting Page', 'buzzstore'); ?></a></p>
                        </div>
                    </div>

                    <a href="<?php echo wp_nonce_url(add_query_arg('buzzstore_hide_notice', 1), 'buzzstore_hide_notice_nonce', '_buzzstore_notice_nonce'); ?>" class="notice-close"><?php esc_html_e('Dismiss', 'buzzstore'); ?></a>
                </div>
        ...
        public function hide_admin_notice() {
            if (isset($_GET['buzzstore_hide_notice']) && isset($_GET['_buzzstore_notice_nonce']) && current_user_can('manage_options')) {
                if (!wp_verify_nonce(wp_unslash($_GET['_buzzstore_notice_nonce']), 'buzzstore_hide_notice_nonce')) {
                    wp_die(esc_html__('Action Failed. Something is Wrong.', 'buzzstore'));
                }

                update_option('buzzstore_hide_notice', true);
            }
        }

        /** Register Menu for Welcome Page */
        public function welcome_register_menu() {
            add_menu_page(esc_html__('Welcome', 'buzzstore'), sprintf(esc_html__('%s', 'buzzstore'), esc_html(str_replace(' ', '', $this->theme_name))), 'manage_options', 'buzzstore-welcome', array($this, 'welcome_screen'), '', 2);
        }
        ...
        public function welcome_styles_and_scripts($hook) {
            if ('theme-install.php' !== $hook) {
                $importer_params = array(
                    'installing_text' => esc_html__('Installing Demo Importer Plugin', 'buzzstore'),
                    'activating_text' => esc_html__('Activating Demo Importer Plugin', 'buzzstore'),
                    'importer_page' => esc_html__('Go to Demo Importer Page', 'buzzstore'),
                ...
                wp_enqueue_style('buzzstore-welcome', get_template_directory_uri() . '/welcome/css/welcome.css', array());
                wp_enqueue_script('buzzstore-welcome', get_template_directory_uri() . '/welcome/js/welcome.js', array('plugin-install', 'updates'), '', true);
        ...
        public function admin_footer_text($text) {
            ...
            if ('appearance_page_buzzstore-welcome' == $screen->id) {
                $text = sprintf(esc_html__('Please leave us a %s rating if you like our theme . A huge thank you from SparkleThemes in advance!', 'buzzstore'), '<a href="https://wordpress.org/support/theme/buzzstore/reviews/?filter=5#new-post" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>');
            }
        ...
        public function generate_demo_installer_button() {
            ...
            if ($this->check_plugin_installed_state($slug, $filename) && !$this->check_plugin_active_state($slug, $filename)) {
                $import_class = 'button button-primary buzzstore-activate-plugin';
                $import_button_text = esc_html__('Activate Demo Importer Plugin', 'buzzstore');
            } elseif ($this->check_plugin_installed_state($slug, $filename)) {
                $import_class = 'button button-primary';
                $import_button_text = esc_html__('Go to Demo Importer Page', 'buzzstore');
                $import_url =  admin_url( '/themes.php?page=sparkle-theme-demo-importer' );
                
            } else {
                $import_class = 'button button-primary buzzstore-install-plugin';
                $import_button_text = esc_html__('Install Demo Importer Plugin', 'buzzstore');
            }
        ...
        public function erase_hide_notice() {
            delete_option('buzzstore_hide_notice');
        }
