...
jQuery( document ).ready(function($) {
	...
	$( '.shopix-devices-preview' ).find( 'button' ).on( 'click', function( event ) {
		...
		if( 'desktop' == device ) {
			device = 'desktop';
			var deviceControl = $( this ).parents( '.customize-control-responsive_number' ).find('.shopix-responsive-wrapper' ).find( 'div' );

			

			deviceControl.each(function( index ) {
				if ( $(this).hasClass( 'shopix-preview-' + device ) ) {
	...
	$(' .wp-full-overlay-footer .devices button ').on('click', function() {
		...
		$( '.shopix-devices-preview li' ).removeClass( 'active' );
		$( '.shopix-devices-preview .' + device ).addClass( 'active' );

		$( '.shopix-responsive-wrapper div' ).removeClass( 'active' );
		$( '.shopix-responsive-wrapper div.shopix-preview-' + device ).addClass( 'active' );
	});	
...
jQuery( document ).ready(function($) {
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.erfontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.erfontslist[index].variants, function(val, text) {
		...
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.erfontslist[index].category);

		shopixGetAllSelects($(this).parent().parent().parent().parent());
	});
	...
	function shopixGetAllSelects($element) {
		var selectedFont = {
...
jQuery( document ).ready(function($) {
	...
	$('.shopix-sortable_repeater_control').each(function() {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					shopixAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.shopix-sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			shopixGetAllInputs($(this).parent());
		}
	});

	// Remove item starting from it's parent element
	$('.shopix-sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				shopixGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			shopixGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	$('.customize-control-sortable-repeater-add').click(function(event) {
		event.preventDefault();
		shopixAppendRow($(this).parent());
		shopixGetAllInputs($(this).parent());
	});

	// Refresh our hidden field if any fields change
	$('.shopix-sortable_repeater.sortable').change(function() {
		shopixGetAllInputs($(this).parent());
	})

	// Append a new row to our list of elements
	function shopixAppendRow($element, defaultValue = '') {
	...
	function shopixGetAllInputs($element) {
...
jQuery( document ).ready(function($) {
	...
	var tabs = [ 'main_header_tabs', 'top_bar_tabs' ];

	$.each( tabs, function( i, val ) {
		$( '.shopix-customizer-tabs[data-tabs-id="' + val + '"]' ).find( 'a' ).on('click', function() {
		
			if ( $( this ).hasClass( 'tab1' ) ) {
				$( ' .tab1' ).addClass( 'active' );
				$( '.shopix-customizer-tabs[data-tabs-id="' + val + '"] .tab2' ).removeClass( 'active' )
			}
	
			if ( $( this ).hasClass( 'tab2' ) ) {
				$( '.shopix-customizer-tabs[data-tabs-id="' + val + '"] .tab1' ).removeClass( 'active' );
				$( '.shopix-customizer-tabs[data-tabs-id="' + val + '"] .tab2' ).addClass( 'active' )
			}			
		});
	});
